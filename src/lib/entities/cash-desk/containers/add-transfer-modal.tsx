...
interface AddTransferModalProps extends DebtorAddCashRegisterEntryParams {
  open: boolean;
...
export const AddTransferModal: FC<AddTransferModalProps> = ({
  ...
  useEffect(() => {
    if (selectedBank && selectedBank !== 'Другое') {
      ...
      // Clear bank_name when switching to "Другое" if not already set
  ...
  const onSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        await refreshShift();
        onOpenChange(false);
        // Call onSuccess after a small delay to ensure data is updated
        setTimeout(() => {
          onSuccess?.();
        }, 100);
      } catch (error) {
    ...
    [handleSubmit, onOpenChange, refreshShift, onSuccess]
  );
